...
def compare_consensus_equal(f1, f2):
    ...
    """"""
    n1 = [i for i in glob.glob(f1)]
    n2 = [i for i in glob.glob(f2)]
...
def compare_consensus(f1, f2):
    ...
    """"""
    n1 = [i for i in glob.glob(f1) if 'FR3' not in i]
                    ...
                    if compare_consensus(cons1, cons2):
                        ...
                        if not minallele in drem:
                            ...
                            if drem[e][minallele] in drem:
                                ...
                                if int(dall[e][maxallele].split(',')[3]) > int(dall[e][maxxallele].split(',')[3]):
                                    drem[e][minallele] = maxallele
                                        
        
                        if not maxallele in drep[e]:
                            drep[e][maxallele] = {}
                            drep[e][maxallele]['nreads'] = int(dall[e][j].split(',')[3]) + int(dall[e][g].split(',')[3])
                            drep[e][maxallele]['nlead'] = int(dall[e][j].split(',')[4]) + int(dall[e][g].split(',')[4])
                            drep[e][maxallele]['nfr1'] = int(dall[e][j].split(',')[5]) + int(dall[e][g].split(',')[5])
                            drep[e][maxallele]['nfr2'] = int(dall[e][j].split(',')[6]) + int(dall[e][g].split(',')[6])
                            drep[e][maxallele]['nfr3'] = int(dall[e][j].split(',')[7]) + int(dall[e][g].split(',')[7])
                            drep[e][maxallele]['comb'] = dall[e][maxallele].split(',')[2].split(';')
                            drep[e][maxallele]['comb'].append(minallele)
                        else:
                            drep[e][maxallele]['nreads'] = int(drep[e][maxallele]['nreads']) + int(dall[e][minallele].split(',')[3])
                            drep[e][maxallele]['nlead'] = int(drep[e][maxallele]['nlead']) + int(dall[e][minallele].split(',')[4])
                            drep[e][maxallele]['nfr1'] = int(drep[e][maxallele]['nfr1']) + int(dall[e][minallele].split(',')[5])
                            drep[e][maxallele]['nfr2'] = int(drep[e][maxallele]['nfr2']) + int(dall[e][minallele].split(',')[6])
                            drep[e][maxallele]['nfr3'] = int(drep[e][maxallele]['nfr3']) + int(dall[e][minallele].split(',')[7])
                            drep[e][maxallele]['comb'].append(minallele)               
